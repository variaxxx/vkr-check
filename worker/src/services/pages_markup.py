...
class MarkupPages:
    ...
    async def markup_pdf(self, pdf_bytes: io.BytesIO) -> Tuple[bool, List[int]]:
        ...
        prompt_text = (
            {
                    "Определи, везде ли на этом изображении проставлены подписи рядом с фамилиями(Оценивай нестрого)? Ответь '1', "
                    "если проставлены все подписи(блок без фамилии не считается за отсутствие подписи), либо '0', если не все подписи проставлены. "
                    "Важно: Если стоит пустой блок и нет фамилии, то это '1', если стоит пустой блок и рядом есть фамилия, то это '0' сразу, если стоит фамилия без блока, то это '1'"
                    "Если стоят несколько блоков с одинаковыми фамилиями, то оценивай их по отдельности, если хотя бы в одном блоке с этой фамилией не стоит подпись, то это '0'",
                }
        )
