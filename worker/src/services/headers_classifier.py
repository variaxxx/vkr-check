...
class HeaderClassifier:
    ...
    def classify_headers(
        self, extracted_chunks: List[Dict[str, str]]
    ) -> List[Dict]:
        ...
        messages = [
            SystemMessage(
                content="Ты — эксперт-аналитик. Отвечаешь строго в формате JSON без пояснений."
            ),
        ...
        try:
            ...
            return [
                {"category": "other", "title": c["header"], "text": c["text"]}
                for c in extracted_chunks
            ]

    def _build_final_structure(
        self, chunks: List[Dict], mapping: List[Dict]
    ) -> List[Dict]:
