...
class InfoParser:
    ...
    async def get_fio(self, pdf_file: io.BytesIO) -> List[str]:
        ...
        raw_list = [p.strip() for p in raw_text.split("\n") if len(p.strip()) > 10]
    ...
    def deduplicate_students(self, students: List[str]) -> List[str]:
        ...
        for s in students:            
            ...
            if '-' in s:
                ...
                group_part = re.sub(r'[Г6Б]ИВ\s?', 'БИВ', group_part, flags=re.IGNORECASE)
        ...
        unique_students = []
        sorted_candidates = sorted(list(set(filtered_list)), key=len, reverse=True)
        ...
        for current in sorted_candidates:
            ...
            for existing in unique_students:
                ...
                if curr_words.issubset(exist_words) or exist_words.issubset(curr_words):
                    is_duplicate = True
                    break
