...
class RAGEngine:
    ...
    def create_vector_db(self, classified_chunks: List[Dict]) -> FAISS:
        ...
        for item in classified_chunks:
            header_prefix = f"Раздел: {item['category']} | Заголовок: {item['title']}\n"
    ...
    def retrieve_relevant_chunks(self, vector_db: FAISS, query: str, k: int = None, categories: List[str] = None) -> List[Document]:
        ...
        search_kwargs = {}
        if categories:
            search_kwargs["filter"] = lambda meta: meta.get("category") in categories
